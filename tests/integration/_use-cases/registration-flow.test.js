...
describe("Use case: Registration Flow (all successful)", () => {
    ...
    const createUserResponse = await fetch(
      "http://localhost:3000/api/v1/users",
      {
        ...
        body: JSON.stringify({
          ...
          email: "registration.flow@stdout.dev.br",
  ...
  test("Receive activation email", async () => {
    ...
    expect(lastEmail.sender).toBe("<contato@stdout.dev.br>");
    expect(lastEmail.recipients[0]).toBe("<registration.flow@stdout.dev.br>");
    expect(lastEmail.subject).toBe("Ative o seu cadastro na StdOut!");
